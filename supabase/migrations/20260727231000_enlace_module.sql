...
create index if not exists materials_allocation_search_idx
on public.materials(client_id, category_id, material_type)
where quality_status = 'APPROVED' and current_stock > 0;

create index if not exists allocations_created_idx
